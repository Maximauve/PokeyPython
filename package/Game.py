...
class Whole_Game:
	...
	def Show_Cards(self, nb):
		validAns = False
		while validAns == False:
			confirm = input(
				self.players[nb].name + ", Voulez vous voir vos cartes? (yes/no)")
			if confirm == "yes" or confirm == "oui" or confirm == "ok":
				self.players[nb].info()
				validAns = True
	...
	def Round(self, nb):
		...
		while end == False:
			if nb == 1:
				...
				print(self.players[self.CurrentPlayer()].name + "A vous de jouer !")
